...
final class NetworkStateAttributeProcessor: AttributeProcessor {
    func appendAttributes(_ attributes: inout [String: Any?]) {
        attributes[Attribute.networkType] = getNetworkType()
        attributes[Attribute.networkGeneration] = getNetworkGeneration()
        attributes[Attribute.networkProvider] = getNetworkProvider()
    }

    private func getNetworkType() -> String {
        guard let reachability = SCNetworkReachabilityCreateWithName(kCFAllocatorDefault, "www.google.com") else {
            return AttributeConstants.noNetwork
        }
        ...
        if isReachable {
            if isWWAN {
                ...
                return AttributeConstants.cellular
            } else {
                return AttributeConstants.wifi
            }
        } else {
            return AttributeConstants.noNetwork
        }
    ...
    private func getNetworkGeneration() -> String {
        ...
        if #available(iOS 14.1, *) {
            ...
            case CTRadioAccessTechnologyGPRS, CTRadioAccessTechnologyEdge, CTRadioAccessTechnologyCDMA1x:
                return AttributeConstants.generation2
            case CTRadioAccessTechnologyLTE:
                return AttributeConstants.generation4
            case CTRadioAccessTechnologyNRNSA, CTRadioAccessTechnologyNR:
                return AttributeConstants.generation5
            default:
                return AttributeConstants.generation3
            }
        } else {
            switch carrierTypeName {
            case CTRadioAccessTechnologyGPRS, CTRadioAccessTechnologyEdge, CTRadioAccessTechnologyCDMA1x:
